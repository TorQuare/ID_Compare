    ...
    public class IdCompareEngine
    {
            ...
            foreach (string value in array_to_compare)
            {
                foreach (string base_value in base_array)
                {
                    if (value == base_value)
                    {
                        ...
                        {
                            case true:
                                if (this.double_check(value, found_duplicates) == false)
                                {
        ...
        private void start_compare()
        {
            ...
            for(int iterator = 0; iterator <= 1; iterator++)
            {
                if (skip_duplicates)
                {
                    string[] not_found = this.not_found();
                }
                string[] found_duplicates = this.compare_duplicates(skip_duplicates);
